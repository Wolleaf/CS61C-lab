...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for (; i + 4 < NUM_ELEMS; i += 4) {
            ...
            __m128i truth_value = _mm_cmpgt_epi32(tmp, _127);
            tmp = _mm_and_si128(tmp, truth_value);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for (; i + 4 * 4 < NUM_ELEMS; i += 4 * 4) {
            ...
            __m128i truth_value1 = _mm_cmpgt_epi32(tmp1, _127);
            __m128i truth_value2 = _mm_cmpgt_epi32(tmp2, _127);
            __m128i truth_value3 = _mm_cmpgt_epi32(tmp3, _127);
            __m128i truth_value4 = _mm_cmpgt_epi32(tmp4, _127);
            tmp1 = _mm_and_si128(tmp1, truth_value1);
            tmp2 = _mm_and_si128(tmp2, truth_value2);
            tmp3 = _mm_and_si128(tmp3, truth_value3);
            tmp4 = _mm_and_si128(tmp4, truth_value4);
